...
struct Box : Shape {
	...
	Box(Point xy, int wid, int hei, int arc) :w{ wid }, h{ hei } , a{arc}
	{
...
struct Regular_polygon : Shape {
	Regular_polygon(Point xy, int num, int dis) :n{ num }, r{ dis }
	{
		add(xy);
	}
...
struct Star : Shape {
	Star(Point xy, int num, int dis1, int dis2) :n{ num }, r1{ dis1 }, r2{dis2}
	{
		add(xy);
	}
