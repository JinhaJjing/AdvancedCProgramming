...
void Box::draw_lines() const
{
	...
	if (color().visibility()) { // edge on top of fill
		...
		fl_arc((point(0).x), (point(0).y), a + a - 1, a + a - 1, 90, 180);
		fl_arc((point(0).x)+w-2*a, (point(0).y), a + a - 1, a + a - 1, 0, 90);
		fl_arc((point(0).x)+w-2*a, (point(0).y)+h-2*a, a + a - 1, a + a - 1, 270, 360);
		fl_arc((point(0).x), (point(0).y)+h-2*a, a + a - 1, a + a - 1, 180, 270);
	}
}
...
void Regular_polygon::draw_lines() const
{
	...
	if (color().visibility()) {	// edge on top of fill
		...
		for (int i = 0; i < n; i++)
		{
			double t1 = i * 2 * 3.14 / n;
			double t2 = (i+1) * 2 * 3.14 / n;
...
void Star::draw_lines() const
{
	...
	if (color().visibility()) {	// edge on top of fill
		...
		{
			double t1 = i* 3.14 / n ;
			double t2 = (i + 1)* 3.14 / n;
			double t3 = (i + 2)*3.14 / n;
			fl_line((point(0).x) + r2 * cos(t1), (point(0).y) + r2 * sin(t1), (point(0).x) + r1 * cos(t2), (point(0).y) + r1 * sin(t2));
			fl_line((point(0).x) + r1 * cos(t2), (point(0).y) + r1 * sin(t2), (point(0).x) + r2 * cos(t3), (point(0).y) + r2 * sin(t3));
		}
	}
}
